...
def compare_templates(R_est, R_t):
    """
    ...
    diff, theta_diff = angle_difference(aa_est, aa_t_array)
    # Normalize Difference Vectors to [0,1]
    diff_n = diff / np.max(diff)
    theta_diff_n = theta_diff / np.max(theta_diff)
    w = [0.9, 0.1]
    sorted_idx = np.argsort(w[0]*diff_n + w[1]*theta_diff_n)

    # Check if Template makes sense
    # sorted_idx = np.argsort(diff)

    # If upper method fails
    return sorted_idx[0]
...
def rot_matrix_to_angle_axis(rotation_matrix):
    ...
    """
    # Derive Alpha, Beta, Gamma and find other way to get theta and v
...
def angle_difference(angle_axis1, angle_axis_array):
    ...
    """
    axes1, theta1 = angle_axis1
    axes2, theta2 = angle_axis_array

    dot_products = np.dot(axes2, axes1)
    ...
    # Compute Theta Difference for Error Correction
    theta_diff = np.abs(theta1 - theta2)

    return angle_differences, theta_diff
...
if __name__ == '__main__':
    ...
    data = np.load("./obj_poses.npy")[:, :3, :3]
    x = compare_templates(R_est, data)
    print("fin")
