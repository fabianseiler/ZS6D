...
def lm(p, t, y_dat):
    ...
    JtWJ, JtWdy, X2, y_hat, J = lm_matx(t, p_old, y_old, 1, J, p, y_dat, weight,
                                        dp)
    ...
    while not stop and iteration <= MaxIter:
        ...
        if (rho > epsilon_4):
            ...
            JtWJ, JtWdy, X2, y_hat, J = lm_matx(t, p_old, y_old, dX2, J, p,
                                                y_dat, weight, dp)
        ...
        # it IS NOT better
            ...
            if not np.remainder(iteration, 2 * Npar):
                JtWJ, JtWdy, dX2, y_hat, J = lm_matx(t, p_old, y_old, -1, J, p,
                                                     y_dat, weight, dp)
    ...
    JtWJ, JtWdy, X2, y_hat, J = lm_matx(t, p_old, y_old, -1, J, p, y_dat,
                                        weight, dp)
...
def make_lm_plots(x, y, cvg_hst):
    ...
    for i in range(n):
        ax2.plot(cvg_hst[:, 0], p_hst[:, i] / p_hst[0, i], color=colors[i],
                 marker=markers[i],
